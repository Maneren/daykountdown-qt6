...
QJsonDocument KountdownExporter::createJsonFromDatabase() const
{
    ...
    while (query.next()) {
        const QJsonObject kountdown{{Database::Columns::NAME_NAME, query.value(Database::Columns::NAME).toString()},
...
{
    const QString fileName = QFileDialog::getSaveFileName(nullptr, i18n("Save File As"), Files::DEFAULT_EXPORT_NAME, Files::JSON_FILTER);
...
void KountdownImporter::parseJsonDocument(const QJsonDocument &doc)
{
    ...
    for (const QJsonValue &kountdownValue : kountdownsArray) {
        ...
        KountdownData data{.name = obj.value(Database::Columns::NAME_NAME).toString(),
                           .description = obj.value(Database::Columns::DESCRIPTION_NAME).toString(),
                           .date = obj.value(Database::Columns::DATE_NAME).toString(),
                           .colour = obj.contains(Database::Columns::COLOUR_NAME) ? obj.value(Database::Columns::COLOUR_NAME).toString() : UI::Colors::DEFAULT};
